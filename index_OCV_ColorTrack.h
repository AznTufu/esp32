  ...
  <div class = "row"> 
      ...
      <div class="section">
        ...
        <table>
            <tr>
                ...
                <td>         <button id="pixelateButton">Toggle Pixelation</button> </td>
            </tr>
        </table>
    </div>
    <div class="section">
    ...
    <div class = "column">      
        ...
        <div class="section">
            <table>
                <tr>
                    <td style="display:none;"><button type="button" id="contourButton" class="btn btn-primary">SHOW CONTOUR</button></td>
                    <td style="display:none;"><button type="button" id="trackButton" class="btn btn-primary">TRACKING</button></td>
                </tr>
                <tr>
                    <td style="display:none;">Contour: <span id="CONTOURdemo"></span></td>
                    <td style="display:none;">Track: <span id="TRACKdemo"></span>
                    </td>
                </tr>
            </table>
        </div>
...
var A=0;
        ...
        var context = canvas.getContext("2d");
        var ShowImage = document.createElement('img'); // Temporary image element to hold the captured image
        var isPixelated = false; // Variable to keep track of pixelation state
        
         document.getElementById('pixelateButton').onclick = function() {
            isPixelated = !isPixelated; // Toggle the pixelation state
            applyEffect();
        };

        function applyEffect() {
            if (isPixelated) {
                pixelateImage(ShowImage);
            } else {
                drawOriginalImage(ShowImage);
            }
        }
        ...
        function loadImage(src, callback) {
            ShowImage.onload = function() {
                ...
                drawOriginalImage(ShowImage);
                // Ensuite, appliquer l'effet en fonction de l'état actuel
                applyEffect();
                // Appeler le callback
                callback();
            };
        ...
        function pixelateImage(img) {
            ...
            let scaleFactor = 10; // Lower the factor for higher pixelation
            let dsize = new cv.Size(src.cols / scaleFactor, src.rows / scaleFactor);
            
            // Downsample the image
            cv.resize(src, lowRes, dsize, 0, 0, cv.INTER_NEAREST);
            
            // Upsample the image back to original size
            cv.resize(lowRes, upscaled, new cv.Size(src.cols, src.rows), 0, 0, cv.INTER_NEAREST);
            
            // Show the pixelated image
            cv.imshow('imageCanvas', upscaled);
        ...
        // Load an example image to test the functionality (replace with actual image source in practice)
        loadImage('example.jpg', function() {
            // Set canvas size to match the image size
            canvas.width = ShowImage.width;
            canvas.height = ShowImage.height;
            // Initial draw of the original image
            drawOriginalImage(ShowImage);
        });
